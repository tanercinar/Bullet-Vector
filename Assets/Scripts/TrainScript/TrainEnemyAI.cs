...
public class TrainEnemyAI : MonoBehaviour
{
    [Header("Referanslar")]
    private Enemy enemy;
    private QBrain brain;
    
    [Header("Karar Ayarları")]
    public float decisionInterval = 0.2f;
    private float decisionTimer = 0f;
    
    [Header("Algılama")]
    public float visionRange = 50f;
    public float visionAngle = 120f; // 60'tan 120'ye genişletildi - daha geniş görüş açısı
    ...
    [Header("Eğitim Eşiği")]
    public float trainedThreshold = 0.1f;
    ...
    void Start()
    {
        ...
        if (brain == null)
        {
            Debug.LogError($"[EnemyAI] {gameObject.name} üzerinde QBrain bileşeni yok!");
            return;
        }
        
        // Tüm aksiyonları kaydet
    ...
    void UpdateSensing()
    {
        ...
        bool isTrainingMode = brain.GetExplorationRate() > trainedThreshold;
        string targetTag = isTrainingMode ? "Enemy" : "Player";
        
        // Debug sadece ara sıra
        if (Random.value < 0.01f)
        {
            Debug.Log($"[ALGILAMA] Mod: {(isTrainingMode ? "EĞİTİM" : "SAVAŞ")} | Aranan: {targetTag} | ε: {brain.GetExplorationRate():F4}");
        }
        ...
        if (potentialTargets.Length == 0)
        {
            if (Random.value < 0.01f)
            {
                Debug.LogWarning($"[ALGILAMA] '{targetTag}' etiketli nesne bulunamadı!");
            }
            return;
        }
        
        int maxChecks = Mathf.Min(potentialTargets.Length, maxTargetCheck);
        
        float closestDistance = Mathf.Infinity;
        Transform closestTarget = null;
        
        for (int idx = 0; idx < maxChecks; idx++)
        {
            GameObject obj = potentialTargets[idx];
            ...
            // Görüş açısı kontrolü - daha toleranslı
            ...
            Vector3 rayDir = (obj.transform.position + Vector3.up * 0.5f) - rayStart;
        ...
        if (closestTarget != null)
        {
            ...
            searchTimer = 0f; // Hedef bulundu, arama sıfırla
        }
    }
    ...
    string BuildState()
    {
        ...
        if (canSeeTarget && target != null)
        {
            ...
            // Açı Grupları
            if (angle < 5f) angleBucket = 0;      // Mükemmel hiza
            else if (angle < 15f) angleBucket = 1; // İyi
            else if (angle < 30f) angleBucket = 2; // Orta
            else if (angle < 60f) angleBucket = 3; // Kötü
            else angleBucket = 4;                  // Çok kötü
            
            // Hedef sağda mı solda mı
            Vector3 cross = Vector3.Cross(transform.forward, dirToTarget);
            if (cross.y > 0.1f) angleDirection = 1;       // Sağda
            else if (cross.y < -0.1f) angleDirection = -1; // Solda
            else angleDirection = 0;                       // Ortada
    ...
    void MoveForward()
    {
        ...
        {
            // Hedef yok, keşif için küçük ödül
    ...
    void TurnLeft()
    {
        // SABİT AÇIYLA DÖN - Time.deltaTime kullanma!
    ...
    void EvaluateTurnQuality(int turnDirection)
    {
        if (!canSeeTarget || target == null)
        {
            // Hedef yok, arama için küçük ödül
    ...
    void Shoot()
    {
        ...
        // Hedef varsa ona doğru bak (ateş etmeden önce küçük düzeltme)
        if (canSeeTarget && target != null)
        {
            Vector3 dirToTarget = target.position - transform.position;
            dirToTarget.y = 0;
            ...
            // Küçük açı düzeltmesi (15 derece altı)
            if (angle > 5f && angle <= 30f)
            {
                Quaternion targetRotation = Quaternion.LookRotation(dirToTarget);
                transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, angle * 0.5f);
            }
            
            // ŞİMDİ ATEŞ ET
            enemy.Shoot();
            
            // Yeni açıyı hesapla (düzeltmeden sonra)
        ...
        {
            // Hedef yok ama ateş etti - BÜYÜK CEZA
            enemy.Shoot(); // Yine de ateş et (öğrenme için)
    ...
    void Wait()
    {
        ...
        // Hedef görünüyorsa bekleme KÖTÜ
        if (canSeeTarget)
        {
            brain.GivePunishment(currentState, 5f);
        }
    ...
    public void OnHitTarget()
    {
        ...
        Debug.Log($"[AI] ★★ VURDU! +200 | ε:{brain.GetExplorationRate():F4}");
    }
    ...
    public void OnDie()
    {
        ...
        Debug.Log($"[AI] ✗✗ ÖLDÜ -300 | ε:{brain.GetExplorationRate():F4}");
    }
    
    public void OnKillTarget()
    {
        brain.GiveReward(currentState, 1000f);
        Debug.Log($"[AI] ★★★ ÖLDÜRDÜ! +1000 | ε:{brain.GetExplorationRate():F4} | Durumlar:{brain.GetStateCount()}");
    }
    ...
    void OnDrawGizmos()
    {
        ...
        {
            // Hedefe çizgi
            Gizmos.color = isTrainingMode ? Color.yellow : Color.red;
            ...
            else if (angle < 30f) Gizmos.color = new Color(1f, 0.5f, 0f); // Turuncu
        ...
        // Görüş konisi
        Gizmos.color = isTrainingMode ? new Color(0, 1, 0, 0.3f) : new Color(0, 1, 1, 0.3f);
